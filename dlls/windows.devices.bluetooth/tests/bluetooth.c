...
static void test_BluetoothLEAdvertisementWatcher( void )
{
    ...
    if (SUCCEEDED( hr ))
    {
        ...
        span = (IReference_TimeSpan *)0xdeadbeef;
        hr = IBluetoothSignalStrengthFilter_get_OutOfRangeTimeout( sig_filter, &span );
        todo_wine ok( hr == S_OK, "got hr %#lx.\n", hr );
        todo_wine ok( !span, "got span %p.\n", span );

        span = (IReference_TimeSpan *)0xdeadbeef;
        hr = IBluetoothSignalStrengthFilter_get_SamplingInterval( sig_filter, &span );
        todo_wine ok( hr == S_OK, "got hr %#lx.\n", hr );
        todo_wine ok( !span, "got span %p.\n", span );
